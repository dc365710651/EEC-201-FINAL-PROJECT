function Test10_Twelve()
    % Test9_Five:
    % Speaker recognition for word "Five."

    baseTrainPath = '/Users/solemnity/Documents/EEC 201/Final_Project/GivenSpeech_Data/Twelve_Training_Data';
    baseTestPath  = '/Users/solemnity/Documents/EEC 201/Final_Project/GivenSpeech_Data/Twelve_Test_Data';

    numSpeakers   = 18;
    codebookSize  = 32;  % example size
    codebooks     = cell(numSpeakers, 1);

    % -- Train codebooks for each speaker
    for spk = 1:numSpeakers
        trainFile = fullfile(baseTrainPath, sprintf('Twelve_train%d.wav', spk+1));
        %fprintf(trainFile);
    ...
    correctCount = 0;
    for spk = 1:numSpeakers
        testFile = fullfile(baseTestPath, sprintf('Twelve_test%d.wav', spk+1));
        ...
        fprintf('Twelve-Word: s%d.wav => recognized as Speaker %d\n', spk, predictedSpk);
        if predictedSpk == spk
    ...
    fprintf('Test10_Twelve: Accuracy for "Twelve" across 18 speakers = %.2f%%\n', accuracy);
end
