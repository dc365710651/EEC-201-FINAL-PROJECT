function Test9()
    ...
    % 2) Reads 8 .wav files in the test folder (s1.wav to s8.wav)
    % 3) For each test file, extracts MFCC, computes the total distortion
    %    to each speaker's codebook, finds the best match, and records 
    %    recognition accuracy.
    % 4) Compares to human accuracy externally.

    trainDataPath = '/Users/solemnity/Documents/EEC 201/Final_Project/GivenSpeech_Data/Training_Data_Test9';
    testDataPath  = '/Users/solemnity/Documents/EEC 201/Final_Project/GivenSpeech_Data/Test_Data_Test9';
    ...
    % Suppose test set has groundTruth labels: s1.wav => speaker 1, etc.
    % If that is not correct, adjust as needed.
    numTestFiles = 18;
    correctCount = 0;
    for t = 1:numTestFiles
        fnameTest = fullfile(testDataPath, ['s', num2str(t), '.wav']);
        [testMFCC, ~] = computeMFCC(fnameTest);

        % Evaluate distance for each speaker
    ...
    fprintf('Test9: Accuracy on these %d test files = %.2f%%\n', numTestFiles, accuracy);
    fprintf('You can compare this result to your own manual identification.\n');
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Subfunctions: computeMFCC, lbgTrainCodebook, distance
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
